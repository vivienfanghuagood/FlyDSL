"""FlyDSL Split-KV Decode Attention for RDNA4 (gfx12xx, wave32).

Split-KV flash decoding with WMMA:
  Stage 1: Grid = (batch, num_q_heads, max_kv_splits)
    - Each workgroup processes a subset of KV tokens
    - WMMA for Q@K^T, element-wise P@V
    - Outputs partial att_out[batch, head, split, head_dim] (f32)
    - Outputs att_lse[batch, head, split] (f32, log-sum-exp)

  Stage 2: Grid = (batch, num_q_heads)
    - Merges partial results across splits using log-sum-exp rescaling
    - Outputs final O[batch, head, head_dim] (bf16)
...
Memory layout:
  ...
  att_out: [batch, num_q_heads, max_kv_splits, head_dim]  f32
  att_lse: [batch, num_q_heads, max_kv_splits]  f32
"""
...
def compile_decode_attention_splitkv(
    *,
    ...
    s1_module_name = (
        f"decode_attn_splitkv_s1_h{num_q_heads}_kv{num_kv_heads}_d{head_dim}"
    )
    s2_module_name = (
        f"decode_attn_splitkv_s2_h{num_q_heads}_kv{num_kv_heads}_d{head_dim}"
    )
    ...
    class _Stage1(flir.MlirModule):
        ...
        def init_gpu_module(self):
            ...
            if num_waves > 1:
                ...
                self._smem_wave_out = s1_allocator.allocate_array(
                    f32, num_waves * head_dim
                )
        ...
        def stage1_kernel(
            ...
            arg_att_out: lambda: T.memref(
                DYN, T.f32()
            ),  # [batch * heads * splits * hdim]
            ...
            att_out_rsrc = buffer_ops.create_buffer_resource(
                _unwrap(arg_att_out), max_size=True
            )
            att_lse_rsrc = buffer_ops.create_buffer_resource(
                _unwrap(arg_att_lse), max_size=True
            )
            ...
            splits_rsrc = buffer_ops.create_buffer_resource(
                _unwrap(arg_num_kv_splits), max_size=True
            )
            ...
            kv_splits_i32 = buffer_ops.buffer_load(
                splits_rsrc, cur_batch, vec_width=1, dtype=i32
            )
            ...
            kv_len_per_split = (
                (inner_cdiv + c_min_block_kv - c1) // c_min_block_kv
            ) * c_min_block_kv
            split_kv_start = kv_len_per_split * split_kv_id
            split_kv_end_raw = split_kv_start + kv_len_per_split
            # min(split_kv_end_raw, seq_len_idx)
            split_kv_end = arith.select(
                split_kv_end_raw < seq_len_idx, split_kv_end_raw, seq_len_idx
            )
            ...
            if num_waves > 1:
                ...
                for w in range_constexpr(num_waves):
                    ...
                    for ei in range_constexpr(elems_per_thread):
                        lds_idx = (
                            arith.index(w) * c_hdim + lane * c_ept + arith.index(ei)
                        )
                        w_val = memref.load(smem_out, [arith.as_value(lds_idx)])
                        new_merge_acc.append(
                            merge_acc[ei] * old_scale + w_val * new_scale
                        )
            ...
            # ================================================================
            # Store partial output: att_out[batch, head, split, :] and att_lse[batch, head, split]
            # att_out is normalized (divided by sum) like Triton does
            # att_lse = e_max + log(e_sum)
            # ================================================================
            # att_out layout: [batch, num_q_heads, max_kv_splits, head_dim] f32
            # flat index = ((batch * num_q_heads + head) * max_kv_splits + split) * head_dim + d
            att_out_base = (
                (cur_batch * c_num_q + cur_q_head) * c_max_splits + split_kv_id
            ) * c_hdim
            ...
            # att_lse layout: [batch, num_q_heads, max_kv_splits] f32
            # flat index = (batch * num_q_heads + head) * max_kv_splits + split
            att_lse_off = (
                cur_batch * c_num_q + cur_q_head
            ) * c_max_splits + split_kv_id
            lse_val = final_emax + flydsl_math.log(arith.as_value(final_esum))
            # Only lane 0 writes LSE (all lanes have same value)
            is_lane0 = lane == c0
            # Use select to conditionally store - write from all lanes but only lane 0 matters
            # Actually all lanes have same lse_val, so any lane can write. Just let lane 0 do it.
            # We need a conditional store. Use buffer_store from lane 0 only.
            # Since FlyDSL doesn't have conditional buffer_store easily, we let all lanes write
            # (they all have the same value, so it's fine).
    ...
    class _Stage2(flir.MlirModule):
        ...
        def stage2_kernel(
            ...
            for s in range_constexpr(max_kv_splits):
                ...
                split_end = arith.select(
                    split_end_raw < seq_len_idx, split_end_raw, seq_len_idx
                )
                ...
                lse_val = buffer_ops.buffer_load(
                    att_lse_rsrc, lse_off, vec_width=1, dtype=f32
                )
                ...
                for ei in range_constexpr(elems_per_thread):
                    ...
                    v_val = buffer_ops.buffer_load(
                        att_out_rsrc, v_off, vec_width=1, dtype=f32
                    )
