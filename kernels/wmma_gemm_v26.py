#!/usr/bin/env python3
"""WMMA GEMM kernel v26 for RDNA4 (gfx12xx, wave32).
...
- XOR-swizzle on LDS stores to avoid bank conflicts (no K-padding)

Key differences from v25:
- 4 warps instead of 8 → 2x more work per warp → better WMMA utilization
- No K-padding → smaller LDS footprint (16KB per buffer vs 20KB)
- XOR-swizzle on LDS store addresses for bank-conflict-free reads

LDS layout (per buffer):
  A tile: 128 rows x 32 cols x 2B = 8KB, stored with XOR-swizzle
  B tile: 128 rows x 32 cols x 2B = 8KB, stored with XOR-swizzle
  Total per buffer: 16KB, double-buffered: 32KB

Pipeline: same as v25 (split GMEM load / LDS store with double buffering)
...
def create_wmma_gemm_v26_module(
    M: int,
    N: int,
    K: int,
    in_dtype="bf16",
    ...
    *,
    ...
    class _WmmaGemmV26(flir.MlirModule):
        GPU_MODULE_NAME = "wmma_gemm_v26"
        ...
        def wmma_gemm_v26_kernel(
            ...
            # ============================================================
            # XOR-swizzle helpers for LDS
            # ============================================================
            # For a row-major [M, K] tile stored in LDS with BLOCK_K=32 bf16 elements,
            # each row = 32 bf16 = 64 bytes = 4 x 128-bit loads.
            # LDS has 32 banks x 4 bytes = 128 bytes per bank cycle.
            # A row of 64 bytes spans 16 banks.
            #
            # XOR-swizzle: for store, XOR the K-group index with the row index
            # to distribute writes across banks. For reads, compute the same XOR
            # to reconstruct the address.
            #
            # With LOAD_VEC=8 bf16 = 16 bytes, we have 4 k-groups per row.
            # XOR pattern: k_group_swizzled = k_group XOR (row % 4)
            # This ensures adjacent rows use different bank patterns.
            ...
            # ============================================================
            # Phase 2: Store loaded data to LDS (with XOR-swizzle)
            # ============================================================
        ...
        def __call__(
            ...
            flir.gpu_ext.LaunchFuncOp(
                ["wmma_gemm_v26", "wmma_gemm_v26_kernel"],
                grid_size=(total_blocks, c1, c1),
                block_size=(bk, c1, c1),
                kernel_operands=[A, B_T, C],
            )

    return _WmmaGemmV26(), BLOCK_M, BLOCK_N, BLOCK_K
