#!/usr/bin/env python3
"""WMMA GEMM kernel v12 for RDNA4 (gfx12xx, wave32).

CK-inspired BSkipLDS architecture with sched_group_barrier:
  - A through LDS (single-buffered, cooperative load, padded)
  - B direct from GMEM via preshuffle (register double-buffered)
  - sched_group_barrier hints to interleave WMMA, VMEM_READ, DS_READ, DS_WRITE
  - Software pipelined: B in VGPRs alternates between two buffers
  - sched_barrier(0) to prevent the compiler from reordering our explicit schedule

Loop structure per iteration (following CK BSkipLDS pattern):
  1. Barrier + wait for LDS ready
  2. Phase 1 (rk=0):
     - LDS read A[rk=0] (DS_READ)
     - 16 WMMAs using A[rk=0] and B_cur
     - Interleaved: GMEM load B_next (VMEM_READ)
  3. Phase 2 (rk=1):
     - LDS read A[rk=1] (DS_READ)
     - GMEM load A_next → wait → store to LDS (VMEM_READ + DS_WRITE)
     - 16 WMMAs using A[rk=1] and B_cur
  4. Swap B buffers
...
def create_wmma_gemm_v12_module(
    M: int,
    N: int,
    K: int,
    in_dtype="bf16",
    ...
    *,
    ...
    assert reg_k == 2, "v12 requires reg_k=2"
    ...
    # A LDS layout (padded, single buffer with barriers)
    BLOCK_K_PAD_A = BLOCK_K + a_k_pad  # 40
    LDS_A_ELEMS = BLOCK_M * BLOCK_K_PAD_A
    ...
    # Number of B loads per wave per K-tile
    # Each wave loads reg_n * reg_k = 8 B tiles, each 4 VGPRs
    NUM_B_LOADS_PER_WAVE = reg_n * reg_k
    ...
    class _WmmaGemmV12(flir.MlirModule):
        GPU_MODULE_NAME = "wmma_gemm_v12"
        ...
        def wmma_gemm_v12_kernel(
            ...
            # ============================================================
            # MAIN PIPELINE (CK BSkipLDS style)
            # ============================================================
            #
            # Prologue:
            #   Load A[0] from GMEM → wait → store to LDS → barrier
            #   Load B[0] from GMEM (async)
            #
            # Main loop (kt=0..N-2):
            #   [barrier already done from previous iter]
            #
            #   Phase 1 (rk=0):
            #     DS_READ: A[rk=0] from LDS
            #     WMMA: 16 WMMAs with A[rk=0] and B_cur[rk=0]
            #     (interleave VMEM_READ for B_next)
            #
            #   Phase 2 (rk=1):
            #     DS_READ: A[rk=1] from LDS
            #     VMEM_READ: A[kt+1] from GMEM (async)
            #     WMMA: 16 WMMAs with A[rk=1] and B_cur[rk=1]
            #     VMEM_WAIT + DS_WRITE: store A[kt+1] to LDS
            #     BARRIER
            #     swap B_cur = B_next
            ...
            for kt in range(num_k_tiles - 1):
                # Phase 1: LDS read A[rk=0], then compute with B_cur[rk=0]
                a_rk0 = _load_a_from_lds_rk(0)

                # Issue B_next loads (will overlap with Phase 1 WMMAs)
                b_next = _load_b_tile(kt + arith.index(1))

                # Phase 1 WMMAs
                accs = _do_compute_rk(accs, a_rk0, b_vecs[0])

                # Phase 2: LDS read A[rk=1]
                a_rk1 = _load_a_from_lds_rk(1)

                # Issue A_next GMEM loads (async, will overlap with WMMAs)
                ...
                # Barrier 1: ensure ALL waves finished reading LDS before any wave writes
                _barrier()

                # Wait for A_next GMEM loads, then store to LDS
                _wait_vmem()
                _store_a_to_lds(a_gmem)

                # Barrier 2: ensure ALL waves finished writing LDS before any wave reads
                _barrier()
        ...
        def __call__(
            ...
            flir.gpu_ext.LaunchFuncOp(
                ["wmma_gemm_v12", "wmma_gemm_v12_kernel"],
                grid_size=(total_blocks, c1, c1),
                block_size=(bk, c1, c1),
                kernel_operands=[A, B_shuf, C],
            )

    return _WmmaGemmV12(), BLOCK_M, BLOCK_N, BLOCK_K
