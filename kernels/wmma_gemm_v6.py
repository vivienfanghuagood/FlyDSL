#!/usr/bin/env python3
"""WMMA GEMM kernel v6 for RDNA4 (gfx12xx, wave32).

Key improvement: BOTH A and B are pre-shuffled on host for direct GMEM loading.
No LDS, no barriers, no ds_load/ds_store — pure GMEM + WMMA.

Architecture: "preshuffle A + preshuffle B"
  - A is pre-shuffled: A[M,K] -> A_shuf[M0, K0, KLane, MLane, KPack]
    where M0=M/16, K0=K/16, KLane=2, MLane=16, KPack=8
    Lane t loads A_shuf[m0, k0, t/16, t%16, 0:8] -> 8 contiguous bf16 = 1x buffer_load_b128

  - B is pre-shuffled: B[K,N] -> B_shuf[N0, K0, KLane, NLane, KPack]
    Same as v2/v5.

  - No LDS allocation, no barriers, no s_wait_dscnt
  - K-loop is simple: load A tile from GMEM, load B tile from GMEM, compute WMMAs
...
def preshuffle_a_wmma(A_mk):
    ...
    - t%16 selects M-row within 16x16 tile
    - t/16 selects K-half (0 or 1), each half has 8 contiguous elements

    Output layout: A_shuf[M0, K0, KLane, MLane, KPack]
    - M0 = M/16 (number of 16-row M tiles)
    - K0 = K/16 (number of 16-col K tiles)
    - KLane = 2 (lane t/16: which K-half)
    - MLane = 16 (lane t%16: which M-row)
    - KPack = 8 (8 contiguous bf16 elements)

    Lane t reads from: A_shuf[m0, k0, t/16, t%16, 0:8] -> 8 contiguous elements
    """
    import torch
    ...
    # Reshape A[M,K] -> [M0, 16, K0, 2, 8]
    #   M -> M0*16 (M0 tiles of 16 rows)
    #   K -> K0*2*8 (K0 tiles, each 16 cols = 2 halves of 8)
    A_reshaped = A_mk.reshape(M0, 16, K0, 2, 8)

    # Permute to [M0, K0, KLane(2), MLane(16), KPack(8)]
...
# =============================================================================
# Host-side pre-shuffle for B (same as v2/v5)
# =============================================================================
...
def create_wmma_gemm_v6_module(
    M: int,
    N: int,
    K: int,
    in_dtype="bf16",
    ...
    *,
    ...
    """Create WMMA GEMM v6 module with pre-shuffled A and B.

    No LDS, no barriers. Both A and B loaded directly from GMEM.
    """
    # Derived constants
    ...
    class _WmmaGemmV6(flir.MlirModule):
        GPU_MODULE_NAME = "wmma_gemm_v6"
        GPU_MODULE_TARGETS = [f'#rocdl.target<chip = "{gpu_arch}">']

        @flir.kernel
        def wmma_gemm_v6_kernel(
            self: flir.T.i64,
            ...
            def _load_a_tile(k_tile_idx):
                ...
                For each (rk, rm) combination, lane t loads:
                  A_shuf[m0, k0, t/16, t%16, 0:8]
                where m0 = tile_m0/16 + wave_m*reg_m + rm
                ...
                for rk in range_constexpr(reg_k):
                    ...
                    for rm in range_constexpr(reg_m):
                        ...
                        # Offset in elements into A_shuf flat buffer
                        elem_off = (
                        ...
                        # Load 8 bf16 elements = 4 f32 = 128 bits
            ...
            # Main K-loop: simple — just load from GMEM and compute
        ...
        def __call__(
            ...
            flir.gpu_ext.LaunchFuncOp(
                ["wmma_gemm_v6", "wmma_gemm_v6_kernel"],
                grid_size=(total_blocks, c1, c1),
                block_size=(bk, c1, c1),
                kernel_operands=[A_shuf, B_shuf, C],
            )

    return _WmmaGemmV6(), BLOCK_M, BLOCK_N, BLOCK_K
