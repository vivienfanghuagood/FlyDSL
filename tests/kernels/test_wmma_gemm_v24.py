#!/usr/bin/env python3
"""Test and benchmark for WMMA GEMM v24 kernel (CK-style 8 warps)."""
...
from kernels.wmma_gemm_v24 import create_wmma_gemm_v24_module


def test_correctness(M=128, N=128, K=128):
    """Test v24 kernel correctness."""
    print(f"\n{'=' * 60}")
    print(f"v24 correctness test: M={M}, N={N}, K={K}")
    print(f"{'=' * 60}")

    mod, BLOCK_M, BLOCK_N, BLOCK_K = create_wmma_gemm_v24_module(
...
def benchmark(M=4096, N=4096, K=4096):
    """Benchmark v24 kernel."""
    print(f"\n{'=' * 60}")
    print(f"v24 benchmark: M={M}, N={N}, K={K}")
    print(f"{'=' * 60}")
    ...
    print(f"v24 / PyTorch: {tflops / pt_tflops * 100:.1f}%")
