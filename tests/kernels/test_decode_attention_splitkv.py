...
from kernels.wmma_decode_attention_splitkv import compile_decode_attention_splitkv
...
def run_splitkv(
    ...
    att_out = torch.zeros(
        batch, num_heads, max_kv_splits, head_dim, dtype=torch.float32, device=q.device
    )
    att_lse = torch.full(
...
def test_correctness():
    ...
    for kv_len in [128, 256, 512, 1024]:
        ...
        for bs in [1, 4]:
            q, k_buf, v_buf, o, kv_indptr, kv_indices, num_splits, ms, sm_scale = (
            ...
            print(
                f"  BS={bs}, KV={kv_len}, splits={ms}: cos_sim={cos_sim:.6f}, max_diff={max_diff:.4e} [{status}]"
            )
...
def benchmark_gpu_only():
    ...
    for kv_len in [256, 512, 1024]:
        ...
        for bs in [1, 4, 8, 16, 32]:
            ...
            print(
                f"{bs:>4} | {kv_len:>6} | {max_kv_splits:>6} | {gpu_us:>10.1f} | {wall_us:>10.1f}"
            )
        print()
